...
class PipelineOptions(collections.MutableMapping):
    ...
    An instance of this class behaves like a dictionary, but values can either be stored in the
    object, or come from environment variables.
    """
    def __init__(self, options=None, defaults=None, env_prefix=None):
        """
        Initialize PipelineOptions instance.

        Option values come from a combination of the options dictionary, environment variables,
        and the defaults dictionary.  When looking up an option value, the options dictionary has
        highest precedence, then environment variables, and the defaults dictionary is checked
        last.

        :param options: A dictionary of option values
        :param defaults: A dictionary with default option values
        :param env_prefix: Environment variable name prefix.  If not empty, '_' will be appended to
        it.
        """
        if env_prefix:
    ...
    def __iter__(self):
        # Not implemented because options can be looked up lazily by looking at environment
        # variables, so to be correct this would need to iterate over all environment variables
        # with self._env_prefix
...
class EasyPipeline(pipeline.Pipeline):
    REQUIRED_OPTIONS = ('LOG_URI', 'PIPELINE_ROLE', 'PIPELINE_RESOURCE_ROLE', 'EC2_AMI',
                        'EC2_KEYPAIR', 'EC2_SUBNET', 'EC2_SECURITY_GROUP')
    ...
    def __init__(self, name, unique_id, options, desc=None, region='us-west-2'):
        ...
        if 'LOG_URI' not in self.options and 'LOG_URI_TEMPLATE' in self.options:
            self.options['LOG_URI'] = self.options['LOG_URI_TEMPLATE'].format(
                region=region, pipeline_name=name)

        missing_opts = [o for o in self.REQUIRED_OPTIONS if o not in self.options]
        if missing_opts:
            raise ValueError("The following required options are not set: {!r}".format(
                missing_opts))
    ...
    def _ensure_object(self, obj_id, cls, **kwargs):
        """
        Get a pipeline object by id, creating it if it doesn't already exist

        :param obj_id: The id of the object to get or create
        ...
        if obj is None:
            obj = cls(id=obj_id, **kwargs)
            self.add(obj)
        return obj

    def get_defaults(self):
        """
        Get the Default object for this pipeline, which contains default property values that all
    ...
    def get_shell_command_activity(self, obj_id):
        """
        Get the ShellCommandActivity object with id, creating it if necessary.

        Callers will need to set the command property to the command to run,  and the runsOn or
        workerGroup property to configure where it runs.

        :param obj_id: Id of the object
        :return: a ShellCommandActivity
        """
        activity = self._ensure_object(
            obj_id, activities.ShellCommandActivity,
            name=obj_id,
            maximumRetries=0)
        return activity

    def get_schedule(self, obj_id='DefaultSchedule'):
        """
        Gets the schedule object, creating it if necessary.

        When creating the schedule, the new schedule is also set as the default schedule for the
        pipeline, and scheduleType is set to CRON.  Callers should set the period and
        startDateTime properties.

        :param obj_id:  Id of the object
        :return: a Schedule object
        """
        schedule = self._ensure_object(obj_id, base.Schedule, name=obj_id)
        defaults = self.get_defaults()
        defaults.scheduleType = keywords.scheduleType.cron
        defaults.schedule = schedule
        return schedule

    def get_ec2_resource(self, obj_id):
        """
    ...
    def get_sns_alarm(self, obj_id):
        ...
        alarm = self._ensure_object(
            obj_id, actions.SnsAlarm,
            name=obj_id,
            role=self.options['PIPELINE_ROLE'])
        return alarm

    def get_sns_failure_handler(self, obj_id):
        """
        Get an SnsAlarm object specifically configured for use as an onError handler.

        The returned SnsAlarm has preset subject and message property values, which show the
        pipeline name, execution status, date of execution, and a link to dashboard for this
        pipeline.  Callers only need to set the topicArn.
...
def main(setup_func, argv=None, env_prefix='PIPELINE'):
    ...
    Call this function to perform commandline-arg parsing, and then create the pipeline and call
    the user's pipeline setup function.   Here's how you could define a pipeline which just runs
    ...
    :param setup_func: A function which will be called with the pipeline object.  It's responsible
    ...
    The following pipeline options must be defined in order for pipeline definitions to work.
    Options can be defined using the -o option, via config file using the -c option,
    or via environment variables prefixed with {env_prefix}_.
    ...
    Options:
      ...
      -c --config CFG_FILE     A config file in docker Envfile format.  Can be repeated
    ...
    pline_obj = EasyPipeline(name=name, unique_id=unique_id, options=pline_options,
                             desc=description, region=region)
